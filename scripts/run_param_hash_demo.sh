...
LOG_FILE="${ROOT_DIR}/outputs/param_hash_demo.log"
RESP_FILE="${ROOT_DIR}/outputs/param_hash_demo.response.json"
...
echo "Running CVM parameter hash demo (log: ${LOG_FILE})"
cvm \
  --manifest_file "${ROOT_DIR}/manifests/manifest.param_hash_demo.json" \
  --asp_bin "${ROOT_DIR}/asps" \
  --req_file "${ROOT_DIR}/requests/request.param_hash_demo.json" \
