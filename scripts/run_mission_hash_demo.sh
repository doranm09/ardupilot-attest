...
LOG_FILE="${ROOT_DIR}/outputs/mission_hash_demo.log"
RESP_FILE="${ROOT_DIR}/outputs/mission_hash_demo.response.json"
...
echo "Running CVM mission hash demo (log: ${LOG_FILE})"
cvm \
  --manifest_file "${ROOT_DIR}/manifests/manifest.mission_hash_demo.json" \
  --asp_bin "${ROOT_DIR}/asps" \
  --req_file "${ROOT_DIR}/requests/request.mission_hash_demo.json" \
